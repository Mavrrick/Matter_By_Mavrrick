 ...
 *
 * This Govee fan Driver is heavily based on kkossev's Matter driver for retrieval of diagnostic and Matter cluster information. 
 * This instance of the driver is customized to work specifically with the Govee Fan H7105 42" tower Max Fan
 */
...
metadata {
    definition(name: 'Matter Fan driver', namespace: 'Mavrrick', author: 'Mavrrick', singleThreaded: true) {
        capability 'Actuator'
        capability 'Switch'
...
/* void natureMode() {
    value = 1
    speedValue = intToHexStr(value)  
    if (logEnable) log.debug "Activating nature mode"
    List<Map<String, String>> attributeWriteRequests = []
    attributeWriteRequests.add(matter.attributeWriteRequest(device.endpointId, 0x0202, 0x000A, 0x04, speedValue ))
    String cmd = matter.writeAttributes(attributeWriteRequests)            
    sendToDevice(cmd)
}
...
void sleepMode() {
    value = 0
    speedValue = intToHexStr(value)  
    if (logEnable) log.debug "Activateting sleep mode"
    List<Map<String, String>> attributeWriteRequests = []
    attributeWriteRequests.add(matter.attributeWriteRequest(device.endpointId, 0x0202, 0x000A, 0x04, speedValue ))
    String cmd = matter.writeAttributes(attributeWriteRequests)            
    sendToDevice(cmd)
...
void autoMode() {
    value = 5
    speedValue = intToHexStr(value)  
    if (logEnable) log.debug "Activateting auto mode"
    List<Map<String, String>> attributeWriteRequests = []
    attributeWriteRequests.add(matter.attributeWriteRequest(device.endpointId, 0x0202, 0x000, 0x04, speedValue ))
    String cmd = matter.writeAttributes(attributeWriteRequests)            
    sendToDevice(cmd)
}
...
void smartMode() {
    value = 6
    speedValue = intToHexStr(value)  
    if (logEnable) log.debug "Activateting Smart mode"
    List<Map<String, String>> attributeWriteRequests = []
    attributeWriteRequests.add(matter.attributeWriteRequest(device.endpointId, 0x0202, 0x000, 0x04, speedValue ))
    String cmd = matter.writeAttributes(attributeWriteRequests)            
    sendToDevice(cmd)
...
void getInfo() {
    ...
    if (state.deviceType == 'MATTER_OUTLET') {
        ...
        requestAndCollectAttributesValues(endpoint = device.endpointId, cluster = 0x0006, time = 30)    // Boolean State Cluster
        requestAndCollectAttributesValues(endpoint = '02',              cluster = 0x0202, time = 45)    // Power Configuration Cluster
    }
}
...
//        attributePaths.add(matter.attributePath(0x01, 0x0202, 0x01))
        attributePaths.add(matter.attributePath(0x01, 0x0202, 0x02))
        attributePaths.add(matter.attributePath(0x01, 0x0202, 0x03))
//        attributePaths.add(matter.attributePath(0x01, 0x0202, 0x04))
//        attributePaths.add(matter.attributePath(0x01, 0x0202, 0x05))
//        attributePaths.add(matter.attributePath(0x01, 0x0202, 0x06))
...
//        attributePaths.add(matter.attributePath(device.endpointId, 0x0202, 0x0001))         // FanModeSequence
...
//        attributePaths.add(matter.attributePath(device.endpointId, 0x0202, 0x0004))         // SpeedMax
//        attributePaths.add(matter.attributePath(device.endpointId, 0x0202, 0x0005))         // SpeedSetting
//        attributePaths.add(matter.attributePath(device.endpointId, 0x0202, 0x0006))         // SpeedCurrent
...
// 3.2. Fan Cluster 0x0202
@Field static final Map<Integer, String> FanClusterAttributes = [
    0x0000  : 'FanMode',
